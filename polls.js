...
async function fetchPoll(pollId) {
    try {
        ...
        if (!response.ok) {
            ...
            console.error("Failed to fetch poll:", response.status, response.statusText);
            throw new Error(`Error fetching poll: ${errorText}`);
        }
        ...
        if (pollData && pollData.data) {
            console.log("Fetched poll data:", pollData.data);
            return pollData.data;  // Ensure correct data structure
        } else {
            console.error("No data found in the response:", pollData);
            return null;
        }
    } catch (error) {
        console.error("Error fetching poll:", error);
...
async function initializePoll() {
    ...
    if (pollId) {
        // Introduce a small delay before fetching the poll data to account for API delay
        setTimeout(async () => {
            const pollData = await fetchPoll(pollId);
            console.log("Fetched poll data:", pollData);
            ...
            if (pollData) {
                ...
                console.error("Failed to fetch the poll data after creation.");
            }
        }, 2000);  // 2-second delay to allow poll creation to propagate
    } else {
